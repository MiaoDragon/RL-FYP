        ...
        for t in range(T):
            ...
            while True:
                ...
                for exp in experience_reply:
                    ...
                    q = sess.run(Q,feed_dict={x:np.reshape(np.array(s+a+[1]), (1,insize+outsize+1) )})
                    q = float(q)
                    if exp[3] in env.t_states:
                        # end of state, Q value is 0
                        # if leave this as free variable, then will not converge
                        nq = 0.
                    # here use temporal difference idea
                    td = exp[2]+nq-q
                    nextQs.append(q+alpha*td)
                    #print(state_actions)
            ...
            for s in range(env.num_state):
                ...
                Vs[s].append(maxQ)
        #print(ts)
        learning_curve(ts, Vs,'learning_curve_Qnetwork-expectation.png')
