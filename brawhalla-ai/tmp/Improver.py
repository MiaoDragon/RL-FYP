...
class Improver(object):
    ...
    def run(self):
        POPULATE_FLAG = True
        POPULATE_MAX = self.shared['memory'].getCapacity()
        populate_num = 0
        PLOT_FREQ = 50
        # PSEUDO:
        # keep looping:
        ...
        while True:
            ...
            memory.push( (state.numpy(), action.numpy(), FloatTensor([[r]]).numpy(), \
                                        next_state.numpy(), next_action.numpy(), FloatTensor([1-done]).numpy()) )
            self.shared['memory'] = memory
            #self.shared['memory'].push( (state, action, FloatTensor([[r]]), \
